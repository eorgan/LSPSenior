# ExecutaRegraRecursiva

## Assinatura

```lspt
Funcao ExecutaRegraRecursiva(Numero CodReg, Numero End Nivel, Alfa End Texto);
```

## Código
999

## Descrição

**Importante** Esta função não deve ser utilizada. Sua finalidade é apenas para testes internos. <!-- source: https://documentacao.senior.com.br/gestaoempresarialerp/5.10.4/regra_funcoes/executarelatorio.htm -->

## Parâmetros

- **CodReg** (`Numero`) - Entrada
- **Nivel** (`Numero End`) - Saída
- **Texto** (`Alfa End`) - Saída

## Exemplo de Uso

```lspt
@-- Adicione exemplo de uso aqui --@
ExecutaRegraRecursiva();
```

## Fonte

[Documentação oficial Senior](https://documentacao.senior.com.br/gestaoempresarialerp/5.10.4/regra_funcoes/executa-regra-recursiva.htm)

> Gerado automaticamente a partir da documentação oficial da LSP por `generate-functions.js`.
> Arquivos com esta nota são regenerados a cada execução; remova-a para editar manualmente
> sem ser sobrescrito.
